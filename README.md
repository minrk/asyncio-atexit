...
Note that a timeout _alone_ wouldn't have fixed this. Callbacks used to be invoked on the loop
...
must not be killed by it. It's armed _before the first callback runs_, so a callback hanging
...
so the _others still get to run_ (I2); the watchdog bounds the process so a supervisor can
